#KOTLAR:
# what Thomas wrote is very good, and difficutl for me to understand
# I've taken this file, originally Seq::Gene, and modified it, to fit within
# the newer system (single kv for main db, + region db)
# Mostly I've renamed things to camelCase to know what's updated
# and split flanking_sites, transcript_sites, transcript_annotation into separate
# roles

#This is the transcript_annotation replacement
...
#package Seq::Gene;
package Seq::Tracks::GeneTrack::TranscriptAnnotation;
...
# Use our tee_logger, because this can also integrate with web services use Carp qw/ confess /;
use namespace::autoclean;
use Data::Dump qw/ dump /;

#This doesn't do very much use Seq::Site::Gene;
...
# Used only for generating offset, which we no longer dohas chr              => ( is => 'rw', isa => 'Str', required => 1, );
  ...
  for ( my $i = 0; $i < @exon_starts; $i++ ) {
    for ( my $n = 1; $n <= $splice_site_length; $n++ ) {
        ...
        && $exon_starts[$i] - $n < $coding_end )
      {
        ...
        #this now uses our zeroBased method
        #no need to specify abs_pos; 
        my $pos   = $self->zeroBased( $exon_starts[$i] ) - $n;
        # This stores the entire record $gene_site{alt_names} = $self->alt_names;
        # That's not needed I think; we just want to store the site_type
        # and any errors right?
        # ok, and transcript id
        $gene_site{site_type} =
          ( $self->strand eq "+" ) ? 'Splice Acceptor' : 'Splice Donor';
        

        # Do we need to know this? $gene_site{ref_base}      = $self->get_base( $gene_site{abs_pos}, 1 );
        #So if we get the base, what do we do with it? We already know the
        # ref because that's what's present at the locations of interest.
